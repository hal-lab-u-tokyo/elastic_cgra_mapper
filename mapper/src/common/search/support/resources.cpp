...
void PlacementSearchEngine::BuildMRRGCache() {
  ...
  std::sort(
      prisa_resources_.begin(), prisa_resources_.end(), [&](int a, int b) {
        const auto pa = mrrg_.GetNodeProperty(a).position_id;
        const auto pb = mrrg_.GetNodeProperty(b).position_id;
        if (pa.first != pb.first) return pa.first < pb.first;
        ...
        return reverse_row ? pa.second > pb.second : pa.second < pb.second;
      });
...
bool PlacementSearchEngine::SamePhysicalPosition(int lhs_mrrg_node,
                                                 int rhs_mrrg_node) const {
  return mrrg_.GetNodeProperty(lhs_mrrg_node).position_id ==
         mrrg_.GetNodeProperty(rhs_mrrg_node).position_id;
}

bool PlacementSearchEngine::IsPhysicalPositionOccupied(
    int mrrg_node_id, const PlacementState& state) const {
...
bool PlacementSearchEngine::CanOccupyResource(
...
std::vector<int> PlacementSearchEngine::CompatibleResources(
    int dfg_node_id, const PlacementState& state) const {
...
std::vector<int> PlacementSearchEngine::ClosestCompatibleResources(
    int dfg_node_id, int anchor_mrrg_node, const PlacementState& state) const {
...
int PlacementSearchEngine::ResourceDistance(int from_mrrg_node,
...
int PlacementSearchEngine::DirectedResourceDistance(int from_mrrg_node,
                                                    int to_mrrg_node) const {
  if (from_mrrg_node < 0 || to_mrrg_node < 0) return kInfDistance;
...
int PlacementSearchEngine::SpatialStepDistance(int from_mrrg_node,
                                               int to_mrrg_node) const {
  if (from_mrrg_node < 0 || to_mrrg_node < 0) return kInfDistance;
...
int PlacementSearchEngine::ApproximateDirectedResourceDistance(
    int from_mrrg_node, int to_mrrg_node) const {
...
int PlacementSearchEngine::PhysicalDistance(int from_mrrg_node,
                                            int to_mrrg_node) const {
  if (from_mrrg_node < 0 || to_mrrg_node < 0) return kInfDistance;
  return std::max(1, SpatialStepDistance(from_mrrg_node, to_mrrg_node));
}

int PlacementSearchEngine::ResourceFreedom(int mrrg_node_id,
...
bool PlacementSearchEngine::IsCornerResource(int mrrg_node_id) const {
  const auto property = mrrg_.GetNodeProperty(mrrg_node_id);
  const auto config = mrrg_.GetMRRGConfig();
  const bool row_on_border = property.position_id.first == 0 ||
...
double PlacementSearchEngine::IOPlacementScore(int dfg_node_id,
                                               int mrrg_node_id) const {
  const auto property = mrrg_.GetNodeProperty(mrrg_node_id);
...
double PlacementSearchEngine::Placement2DInitialPlacementScore(
    int dfg_node_id, int mrrg_node_id) const {
