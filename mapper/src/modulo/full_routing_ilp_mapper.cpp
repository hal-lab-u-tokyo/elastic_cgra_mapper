...
mapper::FullRoutingILPMapper::FullRoutingILPMapper(
    const std::shared_ptr<entity::DFG> dfg_ptr,
    const std::shared_ptr<entity::MRRG> mrrg_ptr)
    : dfg_ptr_(dfg_ptr), mrrg_ptr_(mrrg_ptr) {}

mapper::FullRoutingILPMapper* mapper::FullRoutingILPMapper::CreateMapper(
    const std::shared_ptr<entity::DFG> dfg_ptr,
    const std::shared_ptr<entity::MRRG> mrrg_ptr) {
  mapper::FullRoutingILPMapper* result = new mapper::FullRoutingILPMapper;
  *result = mapper::FullRoutingILPMapper(dfg_ptr, mrrg_ptr);

  return result;
}

mapper::MappingResult mapper::FullRoutingILPMapper::Execution() {
  ...
  try {
    ...
    for (int dfg_edge_id = 0; dfg_edge_id < dfg_edge_num; dfg_edge_id++) {
      for (int mrrg_edge_id = 0; mrrg_edge_id < mrrg_edge_num; mrrg_edge_id++) {
        std::string var_name = "dfg_edge_route_" + std::to_string(dfg_edge_id) +
    ...
    for (int mrrg_node_id = 0; mrrg_node_id < mrrg_node_num; mrrg_node_id++) {
      ...
      for (int dfg_node_id = 0; dfg_node_id < dfg_node_num; dfg_node_id++) {
        ...
        tmp_lin_expr.addTerms(
            &exclusivity_coefficient,
            &(map_source_to_route_PE[dfg_node_id][mrrg_node_id]), 1);
      }
    ...
    for (int dfg_edge_id = 0; dfg_edge_id < dfg_edge_num; dfg_edge_id++) {
      ...
      for (int mrrg_node_id = 0; mrrg_node_id < mrrg_node_num; mrrg_node_id++) {
        ...
        std::string constr_name = "c_edge_flow_" + std::to_string(dfg_edge_id) +
                                  "_" + std::to_string(mrrg_node_id);
        model.addConstr(out_flow - in_flow, GRB_EQUAL,
        ...
        model.addConstr(in_flow, GRB_LESS_EQUAL, 1, constr_name + "_single_in");
    ...
    for (int dfg_node_id = 0; dfg_node_id < dfg_node_num; dfg_node_id++) {
      for (int mrrg_edge_id = 0; mrrg_edge_id < mrrg_edge_num; mrrg_edge_id++) {
        ...
        for (int dfg_edge_id = 0; dfg_edge_id < dfg_edge_num; dfg_edge_id++) {
          ...
          model.addConstr(map_dfg_edge_to_mrrg_edge[dfg_edge_id][mrrg_edge_id],
                          GRB_LESS_EQUAL,
                          map_source_to_route_edge[dfg_node_id][mrrg_edge_id],
                          "c_source_route_edge_lb_" +
                              std::to_string(dfg_node_id) + "_" +
                              std::to_string(dfg_edge_id) + "_" +
                              std::to_string(mrrg_edge_id));
        }
        std::string constr_name = "c_source_route_edge_ub_" +
                                  std::to_string(dfg_node_id) + "_" +
                                  std::to_string(mrrg_edge_id);
      ...
      for (int mrrg_node_id = 0; mrrg_node_id < mrrg_node_num; mrrg_node_id++) {
        ...
        std::string constr_name = "c_source_route_node_ub_" +
                                  std::to_string(dfg_node_id) + "_" +
                                  std::to_string(mrrg_node_id);
        if (source_edge_count > 0) {
    ...
    for (int mrrg_node_id = 0; mrrg_node_id < mrrg_node_num; mrrg_node_id++) {
      ...
      for (int dfg_node_id = 0; dfg_node_id < dfg_node_num; dfg_node_id++) {
        std::string constr_name = "c_route_node_legality_" +
                                  std::to_string(dfg_node_id) + "_" +
                                  std::to_string(mrrg_node_id);
        model.addConstr(map_source_to_route_PE[dfg_node_id][mrrg_node_id],
                        GRB_EQUAL, 0, constr_name);
    ...
    for (int dfg_edge_id = 0; dfg_edge_id < dfg_edge_num; dfg_edge_id++) {
      int source_dfg_node_id = dfg_edge_vec[dfg_edge_id].source;
      std::unique_ptr<double[]> route_edge_values(model.get(
          GRB_DoubleAttr_X, map_dfg_edge_to_mrrg_edge[dfg_edge_id].data(),
          mrrg_edge_num));
      for (int mrrg_edge_id = 0; mrrg_edge_id < mrrg_edge_num; mrrg_edge_id++) {
...
void mapper::FullRoutingILPMapper::SetLogFilePath(
    const std::string& log_file_path) {
  log_file_path_ = log_file_path;
  return;
}

void mapper::FullRoutingILPMapper::SetTimeOut(double timeout_s) {
  timeout_s_ = timeout_s;
  return;
}

void mapper::FullRoutingILPMapper::SetAcceptFeasibleSolution(
