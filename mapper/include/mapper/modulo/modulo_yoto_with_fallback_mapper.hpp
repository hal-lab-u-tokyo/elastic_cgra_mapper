...
// Diagnostic hybrid: try modulo YOTO placement first, then routing-aware
// fallback placements. Keep this separate from ModuloYOTOMapper so experiments
// can distinguish pure and mixed strategies.
class ModuloYOTOWithFallbackMapper : public IMapper {
 public:
  ModuloYOTOWithFallbackMapper() = default;
  ModuloYOTOWithFallbackMapper(const std::shared_ptr<entity::DFG> dfg_ptr,
