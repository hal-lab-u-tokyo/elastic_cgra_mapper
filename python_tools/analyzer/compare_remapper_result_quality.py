...
remapping_time_s, and a utilization metric. This script compares groups of
...
CANONICAL_UTILIZATION_COLUMN = "utilization"
...
def normalize_utilization(rows, fieldnames):
    source_column = next(
        (column for column in UTILIZATION_COLUMNS if column in fieldnames),
        None,
    )
    if source_column is None:
        return False
    for row in rows:
        row[CANONICAL_UTILIZATION_COLUMN] = row.get(source_column, "")
    return True
...
def main():
    ...
    baseline_has_utilization = normalize_utilization(
        baseline_rows, baseline_fields
    )
    candidate_has_utilization = normalize_utilization(
        candidate_rows, candidate_fields
    )
    utilization_columns = (
        [CANONICAL_UTILIZATION_COLUMN]
        if baseline_has_utilization and candidate_has_utilization
        else []
    )
