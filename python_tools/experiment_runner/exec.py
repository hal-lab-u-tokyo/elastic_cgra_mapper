...
class MappingInput:
  def __init__(self, dfg_file_path, output_dir_path, cgra, benchmark, timeout_s, parallel_num, overwrite):
...
def mapping_exec(input):
  cgra_dir_path = "./tmp_cgra/mapping/"
  ...
  lock.acquire()
  try:
    experiment_log_file = open(_experiment_log_file_path, "a")
    ...
    experiment_log_file.write("cgra: " + input.cgra.get_cgra_dict().to_string() + "\n")
    experiment_log_file.write("benchmark: " + input.benchmark + "\n")
  ...
  subprocess.run(["../../build/mapping", input.dfg_file_path, cgra_file_path, input.output_dir_path, str(input.timeout_s), str(input.parallel_num)])  

  os.remove(cgra_file)

def create_database_exec(input):
  cgra_dir_path = "./tmp_cgra/database/"
  ...
  subprocess.run(["../../build/create_database", input.dfg_file_path, cgra_file_path, input.output_dir_path, str(input.db_timeout_s)])  
...
def remapper_exec(input):
  ...
  subprocess.run(["../../build/remapper", input.dfg_file_path, cgra_file_path, input.output_dir_path, str(int(input.remapper_mode))])  
