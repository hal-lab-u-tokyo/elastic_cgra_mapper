...
The VTR repository has Verilog/BLIF material for the generated entries, but those files are not always at the same benchmark granularity as the PRISA table. For example, the local `diffeq2` packed net contains 245 top-level VPR blocks, while the PRISA table reports 121. See `normalization_report.md` for the stage-by-stage diagnosis.
