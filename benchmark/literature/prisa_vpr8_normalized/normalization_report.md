...
This means the exact PRISA VPR-8 reproduction still requires the same packed benchmark artifacts or extraction script used by the paper authors. The current `prisa_vpr8` experiment is therefore a mixed set: the first ten entries are paper-granularity or near-paper-granularity DOTs, while the generated VTR entries are stress tests that help check whether the reported large-DFG trends appear.
