...
def prepare_mapper_config_for_run(
    ...
    config_overrides = dict(mapper.get("mapper_config_overrides", {}))
    needs_generated_config = (
        evaluation_mode == "placement_only" or algorithm_overrides or config_overrides
    )
    ...
    mapper_config = deep_merge_dict(mapper_config, config_overrides)
    generated_dir = run_dir / "generated_inputs"
    generated_dir.mkdir(parents=True, exist_ok=True)
...
def expand_mapper_matrix(manifest: dict) -> list:
    matrix = manifest.get("mapper_matrix", {})
    placements = matrix.get("placements", [])
    routers = matrix.get("routers", [])
    name_template = matrix.get("name_template", "{placement}__{routing}")
    result = []

    for placement in placements:
        for router in routers:
            entry = dict(placement)
            entry.update({key: value for key, value in router.items() if key != "algorithm_overrides"})
            placement_name = str(placement["name"])
            routing_name = str(router["name"])
            entry["name"] = name_template.format(
                placement=placement_name,
                routing=routing_name,
            )
            entry["placement_method"] = placement.get("placement_method", placement_name)
            entry["routing_method"] = router.get("routing_method", routing_name)
            entry["mapper_role"] = matrix.get("mapper_role", "placement_then_routing")
            entry["algorithm_overrides"] = deep_merge_dict(
                dict(placement.get("algorithm_overrides", {})),
                dict(router.get("algorithm_overrides", {})),
            )
            result.append(entry)
...
def manifest_mappers(manifest: dict) -> list:
    result = []
    for mapper in manifest.get("mappers", []):
        result.append(dict(mapper))

    result.extend(expand_mapper_matrix(manifest))

    for mapper in manifest.get("routing_aware_mappers", []):
        entry = dict(mapper)
        entry.setdefault("mapper_role", "routing_aware_placement")
        entry.setdefault("placement_method", entry.get("name", ""))
        entry.setdefault("routing_method", "integrated")
        result.append(entry)

    for mapper in manifest.get("external_mappers", []):
        entry = dict(mapper)
        entry.setdefault("mapper_role", "external_baseline")
        entry.setdefault("placement_method", entry.get("name", ""))
        entry.setdefault("routing_method", entry.get("runner", "external"))
        result.append(entry)
...
def write_run_info(result_dir: Path, metadata: dict) -> None:
    ...
    lines.extend(
        [
            ...
            "- `summary_by_placement_method.md`, `summary_by_routing_method.md`, `summary_by_mapper_role.md`: generated when the manifest provides those comparison axes",
            "",
            "## Log Lookup",
            "",
...
def main() -> None:
    ...
    manifest_log["expanded_mappers"] = manifest_mappers(manifest)
    ...
    for index, item in enumerate(run_plan, start=1):
        ...
        summary["mapper_role"] = mapper.get("mapper_role", "")
        ...
        if metrics_path.exists():
            with metrics_path.open() as f:
                for row in csv.DictReader(f):
                    ...
                    row["placement_method"] = mapper.get("placement_method", "")
                    row["routing_method"] = mapper.get("routing_method", "")
                    row["mapper_role"] = mapper.get("mapper_role", "")
                    all_rows.append(row)
