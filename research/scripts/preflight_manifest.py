...
from lib import compute_res_mii, load_json, read_dfg_stats, write_json
from run_suite import expand_benchmark_sets, parse_filter, selected
...
def add_issue_table(lines: list, rows: list) -> None:
    ...
    lines.append("| benchmark set | benchmark | arch | mapper | status | detail |")
    lines.append("| --- | --- | --- | --- | --- | --- |")
    for row in issue_rows:
        ...
        lines.append(
            ...
            f"{row['mapper']} | {row['preflight_status']} | {'; '.join(detail_parts)} |"
        )
    lines.append("")
...
def add_ready_table(lines: list, rows: list) -> None:
    ...
    lines.append("| benchmark set | benchmark | arch | mapper | MII | start II | ii max | max trials | dfg nodes | dfg edges |")
    lines.append("| --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |")
            ...
            for arch in manifest["architectures"]:
                ...
                for mapper in manifest["mappers"]:
                    if not selected(mapper["name"], filters["mapper"]):
                        continue
                    mapper_config_path = resolve_repo_path(mapper["mapper_config"], repo_root)
                    missing_files = []
                    for label, path in [
                        ("dfg", dfg_path),
                        ("arch", arch_path),
                        ("mapper", mapper_config_path),
                    ]:
                        if not path.exists():
                    ...
                    if not missing_files:
                        ...
                        mii_data = compute_res_mii(dfg_path, arch_path, missing_distance_policy)
