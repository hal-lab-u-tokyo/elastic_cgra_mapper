...
entity::MapperConfig io::ReadMapperConfigFromJsonFile(std::string file_name) {
  ...
  if (auto accept_feasible_solution = algorithm_config_ptree.get_optional<bool>(
  ...
  if (auto max_trials =
          algorithm_config_ptree.get_optional<int>("max_trials")) {
    mapper_config.algorithm_config.max_trials = max_trials.get();
  }
  if (auto seed_count =
          algorithm_config_ptree.get_optional<int>("seed_count")) {
  ...
  if (auto random_seed =
          algorithm_config_ptree.get_optional<int>("random_seed")) {
  ...
  if (auto trial_seed_policy = algorithm_config_ptree.get_optional<std::string>(
          "trial_seed_policy")) {
    mapper_config.algorithm_config.trial_seed_policy = trial_seed_policy.get();
  }
...
void io::WriteMapperConfigToJsonFile(
  ...
  if (mapper_config.algorithm_config.max_trials.has_value()) {
    algorithm_config_ptree.put(
        "max_trials", mapper_config.algorithm_config.max_trials.value());
  }
  if (mapper_config.algorithm_config.seed_count.has_value()) {
    algorithm_config_ptree.put(
        "seed_count", mapper_config.algorithm_config.seed_count.value());
  }
  ...
  if (mapper_config.algorithm_config.random_seed.has_value()) {
    algorithm_config_ptree.put(
        "random_seed", mapper_config.algorithm_config.random_seed.value());
  }
